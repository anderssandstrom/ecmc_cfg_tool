...
class SpectrumWindow(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("PV FFT - Frequency Domain")
        self.resize(860, 640)
...
class FftWindow(QtWidgets.QMainWindow):
    def __init__(self, default_prefix="", initial_pvs=None, timeout=2.0):
        super().__init__()
        self.setWindowTitle("PV FFT - Time Domain")
        self.resize(980, 680)
    ...
    def _open_fft_window(self):
        return self._open_script_window("start_fft.sh", "fft", extra_args=self._selected_pvs())
...
def main():
    ap = argparse.ArgumentParser(description="Qt app for timestamp-derived FFT analysis of EPICS PVs")
    ...
    window = FftWindow(default_prefix=args.prefix, initial_pvs=args.pv, timeout=args.timeout)
    window.show()
    sys.exit(app.exec_())
